<div>
    <x-slot name="breadcrumbs">
        <x-ui.breadcrumbs :items="[
            ...
            ['label' => 'إضافة مستخدم جديد', 'icon' => 'fas fa-user-plus'],
        ]" />
    </x-slot>
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <div class="flex items-center justify-between text-xs text-gray-600 dark:text-gray-400">
                <span
                    ...
                    الأساسية</span>
                <span class="{{ $currentStep >= 2 ? 'text-primary-600 dark:text-primary-400 font-medium' : '' }}">إسناد
                    الأدوار</span>
                <span
                    class="{{ $currentStep >= 3 ? 'text-primary-600 dark:text-primary-400 font-medium' : '' }}">التأكيد
                    والحفظ</span>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
            ...
            @if ($currentStep === 1)
                @include('livewire.dashboard.users.steps.user-basic-info-step')
            @elseif ($currentStep === 2)
                @include('livewire.dashboard.users.steps.user-roles-step')
            @elseif ($currentStep === 3)
                @include('livewire.dashboard.users.steps.user-confirmation-step')
            @endif
