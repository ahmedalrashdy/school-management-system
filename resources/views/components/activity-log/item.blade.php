...
>
    ...
    <div class="flex-1 pb-4 sm:pb-8 min-w-0">
        ...
        <div
            ...
            <!-- Header -->
            <div
                class="p-4 sm:p-5 cursor-pointer select-none flex items-start justify-between gap-3 sm:gap-4"
            ...
            >
                ...
                <div class="flex-1 space-y-1.5">
                    ...
                    <div class="flex flex-wrap items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                        <span class="font-semibold text-gray-700 dark:text-gray-200 flex items-center gap-1.5">
                            {{-- User Avatar --}}
                            <div
                                class="w-5 h-5 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center overflow-hidden">
                                <i class="fas fa-user text-[10px] text-gray-400"></i>
                            </div>
                            {{ $activity->causer?->shortName ?? 'النظام' }}
                        </span>
            ...
            >
                <div class="p-4 sm:p-5 ">
                    ...
                    @endif
                    @foreach ($activity->batchChildren ?? [] as $subActivity)
                        <x-activity-log.sub-item
            ...
            <!-- Batch Indicator Footer -->
            @if ($activity->is_batch_root && $activity->batchChildren->count() > 0)
                <div
                    x-show="!expanded"
                    class="px-4 py-2 bg-gray-50 dark:bg-gray-700/50 border-t border-gray-100 dark:border-gray-700 text-xs text-gray-500 flex items-center justify-between cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
                    @click="expanded = true"
                >
                    <span class="flex items-center gap-2">
                        <i class="fas fa-layer-group text-gray-400"></i>
                        <span class="hidden sm:inline">يحتوي على تفاصيل إضافية...</span>
                        <span class="sm:hidden">التفاصيل...</span>
                    </span>
                    <span class="text-primary-600 font-medium">عرض</span>
                </div>
            @endif
        </div>
    </div>
